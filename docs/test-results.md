...
- 현재 `.venv`는 Python 3.9.6이며, 2026-06-09 점검에서 pytest 전체 통과를 확인했다.
...
성공. 2026-06-09 점검에서 테스트 타입 표기 호환성을 보완한 뒤, 샌드박스 밖 로컬 포트 바인딩 허용 환경에서 전체 테스트가 통과했다.
...
샌드박스 내부에서는 로컬 포트 바인딩 권한 때문에 통합 테스트 3개가 `PermissionError`로 실패할 수 있다. 이 경우 기능 오류가 아니라 실행 권한 문제이므로, 로컬 포트 바인딩이 가능한 환경에서 재실행한다.
